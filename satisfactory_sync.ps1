...
if (Test-Path "$env:USERPROFILE\folder_satisfactory.txt") {
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        if ((Test-Path "$env:LOCALAPPDATA\FactoryGame\Saved\SaveGames\$($satisfactoryPath | Split-Path -Leaf)")) {
    ...
    foreach ($file in $fileList) {
        ...
        if (!$fileAlreadyUploaded) {
            try{
                ...
                $sync_error ++
                Show-MessageForm -Message "Fehler beim Download"  -Color "DarkRed"
            }
            $ftpStream.Close()
            $ftpStream.Dispose()
    ...
    while (!$ftpReader.EndOfStream) {
        ...
        if ($fileName -notmatch $env:COMPUTERNAME -and $fileName.EndsWith(".sav") -and $downloadedFiles -notcontains $fileName) {
            ...
            try{
                ...
                Show-MessageForm -Message "Fehler beim Upload"  -Color "DarkRed"
                $sync_error ++
            }
        }
        else{
...
$icon = [System.Drawing.Icon]::ExtractAssociatedIcon("C:\Program Files (x86)\Steam\steamapps\common\Satisfactory\FactoryGame.exe") #Your Tray Icon here
